...
const char *nameByXT(cell xt) {
	DE_T *dp = (DE_T *)last, *end = (DE_T*)&mem[MEM_SZ];
	while (dp < end) {
...
void doSee() {
	...
	while (i < stop) {
		cell op = code[i++];
		zTypeStrNum("\r\n", i-1, 10, 4); zTypeStrNum(": ", op, 10, 4); emit(' ');
		if (op & LIT_MASK) { op &= LIT_BITS; zTypeStrNum("num #", op, 10, 0); continue; }
		x = code[i];
		switch (op) {
			case LIT:     zTypeStrNum("lit #",   x, 10, 0); i++;
			BCASE JMP:    zTypeStrNum("jmp ",    x, 10, 4); i++;
			BCASE JMPZ:   zTypeStrNum("jmpz ",   x, 10, 4); i++;
			BCASE NJMPZ:  zTypeStrNum("njmpz ",  x, 10, 4); i++;
			BCASE JMPNZ:  zTypeStrNum("jmpnz ",  x, 10, 4); i++;
